...
async def main():
    parser = argparse.ArgumentParser(description="Weekly Telegram Pharma/Medical Pipeline")
    ...
    config = load_config()
    print(f"=== Starting Weekly Pharma Pipeline ({datetime.now()}) ===")
    
    # 1. Fetch & Filter
    print(">>> Step 1: Fetching & Filtering (Pharma)...")
    fetcher = Fetcher(config, topic="pharma")
    ...
    subject = f"Telegram Weekly Pharma/Medical – {datetime.now().strftime('%Y-%m-%d')}"
    html = summarizer.render_html(df, subject_line=subject)
    
    if args.dry_run:
        print("    [Dry Run] Saving email preview to preview_medical.html")
        with open("preview_medical.html", "w", encoding="utf-8") as f:
            f.write(html)
    else:
